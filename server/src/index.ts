    ...
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        connectSrc: [
          ...
          'ws://localhost:*',
          'ws://127.0.0.1:*',
          // Production: allow WSS connections to the same host (Render, etc.)
          ...(!isDev ? ['wss:'] : []),
...
// Auto-detect Render external URL for CORS/WebSocket origin validation
if (process.env.RENDER_EXTERNAL_URL && !process.env.CORS_ORIGINS) {
  ALLOWED_ORIGINS.push(process.env.RENDER_EXTERNAL_URL);
  log.info({ renderUrl: process.env.RENDER_EXTERNAL_URL }, 'Added Render external URL to allowed origins');
}
